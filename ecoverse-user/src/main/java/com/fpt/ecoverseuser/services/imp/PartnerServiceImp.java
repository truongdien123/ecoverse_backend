...
import com.fpt.ecoversecommon.exception.FileHandlingException;
...
import com.fpt.ecoverseuser.dtos.*;
...
public class PartnerServiceImp implements PartnerService {
    ...
    @Override
    @Transactional
    public BulkCreateReportResponse bulkCreate(MultipartFile file, String partnerId) {
        Optional<Partner> partner = partnerRepository.findById(partnerId);
        if (!partner.isPresent()) {
            throw new NotFoundException("Not found partner");
        }
        ...
        Set<String> existingEmails = checkingEmailRepository.findExistingEmail(parentEmailInFile);
...
// generate report file
        ...
        BulkCreateReportResponse response = new BulkCreateReportResponse();
